   ...
   public:
    ...
    Node* cloneGraph(Node* node) {
        ...
        if (mp.find(node) == mp.end()) {
            mp[node] = new Node(node->val, {});  // clone
            for (auto adj : node->neighbors) {
                mp[node]->neighbors.push_back(
                    cloneGraph(adj));  // clone neighbors
            }
        }
        return mp[node];
    }
...
class Solution {
   public:
    unordered_map<Node*, Node*>
        mp;  // declaring map, to check whwther we have a copy of node or not
             // and also to store copy
    ...
    Node* cloneGraph(Node* node) {
        ...
        while (q.empty() == false)  // until q. empty == false
        {
            ...
            for (auto adj : curr->neighbors)  // now travel in adjcant
            {
                ...
                mp[curr]->neighbors.push_back(
                    mp[adj]);  // in current node push adjcant node
            }
        }
